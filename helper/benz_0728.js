// This file is the raw file from AD,which be referred by me. 
var pbIndex = 0;
...
$(function(){
	...
	resize();
	$('#t_policy,#t_table').on('click',changePart3)
	$('#t_outstanding,#t_pics').on('click',changePart2)
	$('.left,.right').on('click',changePB)
	$('.spic').on('click',changePS);
	$('#try label').on('click',function(){
		$('#try label.checked').removeClass('checked');
...
function changePS(){
	var _t = $(this);
	var index = _t.index();
	$('.spic.active').removeClass('active');
	_t.addClass('active');
	$('#pics .picImg:visible').fadeOut();
	$('#pics .picImg').eq(index).fadeIn();
	var ct = $('.ct');
	console.log(index);
	switch(index){
		case 0:
		case 1:
			ct.animate({marginLeft:0},300);
		break;
		case 10:
			ct.animate({marginLeft:-1554},300);
		break;
		case 11:
			ct.animate({marginLeft:-1554},300);
		break;
		default:
			ct.animate({marginLeft:-(index-2)*222},300);
		break;
	}
}
...
function changePB(){
	...
	if(_t.hasClass('left')){
		...
		pbIndex = (pbIndex+10)%9;
	}
	$('.points.active').removeClass('active');
	$('#outstanding .picImg:visible').fadeOut();
	$('#outstanding .picImg').eq(pbIndex).fadeIn();
	$('.points').eq(pbIndex).addClass('active');
}

function changePart2(e){
	var _t = $(this);
	var target = _t.attr('id').replace('t_','');
	$('#part2 .title .active').removeClass('active');
	_t.addClass('active')
	$('.pics').hide();
	$('#'+target).show();
}

function changePart3(e){
	var _t = $(this);
	var target = _t.attr('id').replace('t_','');
	$('#part3 .title .active').removeClass('active');
	_t.addClass('active')
	$('.detail img').hide();
	$('#'+target).show();
}
...
function resize(){
	$('#bg').css('left',($(window).width()-2146)*0.4);
}
...
$(function(){
	...
	$.each(City,function(){
		$("<option/>").text(this.name).appendTo($province);
	});
	...
	$province.on("change",function(){
		...
		$.each(City,function(){
			if(this.name==selProv){
				var selProvCities=this.state;
				$.each(selProvCities,function(){
					var oneCity=this.name;
					$("<option/>").text(oneCity).appendTo($city);
				})
			}
		});
		$city.trigger("change");
	...
	$city.on("change",function(){
		...
		$shop.empty();
		if(selCity===-1){
			$shop.append(optionItem());
		}
		$.each(City,function(){
			var selProvCities=this.state;
			$.each(selProvCities,function(){
				if(selCity==this.name){
					var selCityShops=this.state;
					$.each(selCityShops,function(){
						var oneShop=this.name;
						$("<option/>").text(oneShop).appendTo($shop);
					})
				}
			})
		})
	...
	$(".submit").on("click",function(event){
		var name=$.trim($("#name").val()),
			...
			sex=$(":radio:checked").val(),//male:1 female:0
			...
			console.log(JSON.stringify(usedData));
			event.preventDefault();
			subElem.text("正在提交...").css("color","gray").prop("disabled",true);
			$.ajax({
				url:"/eaclient/apijson.php",
				type:"post",
				processData:false,
				data:JSON.stringify(usedData),
				dataType:"json"
			}).done(function(a) {//参见 http://api.jquery.com/deferred.done/，a为返回的数据，相当于xhr.responseText
				console.log(a.body.oelement.errorcode),
				console.log(typeof a.body.oelement.errorcode),
				"0" === String(a.body.oelement.errorcode) ? 
				(console.log("data saved: " + a), applyForm.hide(), regiSuccess.css("display","block")) :
				(console.log(a.body.oelement.errormsg), subElem.text("提交失败，重新提交").css("color", "gray").prop("disabled", false)); ///*$("<p/>").text("提交成功").appendTo($("#try"))
			}).fail(function(e, a) {
				subElem.text("提交失败，重新提交").css("color", "gray").prop("disabled", false),
...
function optionItem() {//生成<option value=1>请选择</option>
	return $("<option/>", {
		value: -1
	}).text("请选择")
}